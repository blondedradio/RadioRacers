...
static void M_StartCup(UINT8 entry)
{
	...
	if (entry == UINT8_MAX)
	{
		...
		// GP Encore?
		const boolean isgpencore = (boolean)cv_dummygpencore.value;
		grandprixinfo.encore = isgpencore;

		// Populate the roundqueue
		memset(&roundqueue, 0, sizeof(struct roundqueue));
		G_GPCupIntoRoundQueue(levellist.levelsearch.cup, levellist.newgametype, isgpencore);
